...
const Menu = () => {
  ...
  const isScreenRepeated = (history, key) => {
    if (!history) return false;
    const existInHistory = history.find((screen) => screen.key === key);
  ...
  return (
    <NavigationContainer
      onStateChange={(state) => {
        const existScreen = isScreenRepeated(
          navigationState?.history,
          getCurrentScreenKey(state)
        );

        const handleHistory = () => {
          const routes = [...navigationState.history];
          if (!existScreen) {
            routes.push({
              key: getCurrentScreenKey(state),
              type: "route",
            });
          }
          return routes;
